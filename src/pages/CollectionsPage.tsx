...
export function CollectionsPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/collections?limit=1000`)
      ...
      .catch(err => {
        console.warn('Could not fetch collections (possibly dev server restart):', err.message);
        setLoading(false);
      });
  }, []);
          ...
          {collections.map((collection, index) => {
            ...
            return (
              <Link
                ...
                className={`group relative rounded-xl overflow-hidden flex flex-col ${className}`}
              >
                <div className="relative flex-1 overflow-hidden bg-gray-100 rounded-xl">
                  <img loading="lazy" 
                    src={image} 
                ...
                {/* Text content */}
                <div className="pt-4 pb-2 z-10 w-full bg-transparent">
                  <h2 className="text-xl md:text-2xl font-serif text-gray-900 mb-1 italic">
                    {collection.title}
                  </h2>
                  {collection.description && (
                    <p className="text-[13px] md:text-[14px] text-gray-500 mb-3 font-sans tracking-wide truncate">
                      {collection.description}
                    </p>
                  )}
                  <div className="text-[12px] md:text-[13px] text-[#8b4e36] font-medium tracking-widest flex items-center gap-1 group-hover:gap-2 transition-all">
                    探索系列 <span>→</span>
                  </div>
                </div>
              </Link>
