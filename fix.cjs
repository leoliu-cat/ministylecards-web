...
const mappings = {
  ...
  'src/pages/JournalPage.tsx': [ 
    { search: "fetch(``)", replace: "fetch(`${API_BASE_URL}/api/posts`)", instance: 0 },
    { search: "fetch(``)", replace: "fetch(`${API_BASE_URL}/api/posts/${post.slug}`)", instance: 1 }
  ],
  ...
  'src/pages/CheckoutPage.tsx': [ { search: "fetch(``, {", replace: "fetch('/api/pay', {" } ],
    ...
    for (const op of ops) {
      if (op.instance !== undefined) {
        ...
        for (let i = 0; i <= op.instance; i++) {
          pos = content.indexOf('fetch(``', pos + 1);
          if (pos === -1) break;
        }
        if (pos !== -1) {
